...
ROOT_DIR = os.path.abspath("../Mask_R-CNN")
...
for frame in filelist:
    ...
    for box, score, label in zip(r['rois'], r['scores'], r['class_ids']):
        if label == 2 or label == 3 or label == 4 or label == 6 or label == 8:
            detect_list.append(
                str(frame_id) + ',' + str(label) + ',' + str(box[1]) + ',' + str(box[0]) + ','
                + str((box[3] - box[1] + 1)) + ',' + str((box[2] - box[0] + 1)) + ',' + str(
                    score) + ',' + str(-1) + ',' + str(-1)
                + ',' + str(-1))

detections = sorted(detect_list, key=lambda x: int(x.split(',')[0]))
detec_file = open('detections_mask_rcnn.txt', 'w')
