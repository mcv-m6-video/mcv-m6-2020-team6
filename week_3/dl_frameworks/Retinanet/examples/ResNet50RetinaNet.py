...
for frame in filelist:
    ...
    for box, score, label in zip(boxes[0], scores[0], labels[0]):
        ...
        if score < 0.5:
            break
        if label == 1 or label == 2 or label == 4 or label == 5 or label == 7:
            detect_list.append(
                str(frame_id) + ',' + str(label) + ',' + str(box[0]) + ',' + str(box[1]) + ','
                + str((box[2] - box[0] + 1)) + ',' + str((box[3] - box[1] + 1)) + ',' + str(
                    score) + ',' + str(-1) + ',' + str(-1)
                + ',' + str(-1))
    """        
...
detec_file = open('detections_retinanet.txt', 'w')
for i in detections:
