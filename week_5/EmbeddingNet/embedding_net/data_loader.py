...
class EmbeddingNetImageLoader:
    ...
    def _load_images_paths(self):
        for d in self.data_subsets:
            ...
            main_root = self.dataset_path + d
            filelist = os.listdir(main_root)
    ...
    def get_batch_triplets_mining(self,
        ...
        for idx, cl_img_idxs in enumerate(selected_images):
            ...
            print('\n num_class:', cl_img_idxs)
            print('\n id:', idx)
            print('\n finito:', np.isfinite(embeddings).all())
            print('\n NAN:', np.isnan(embeddings).all())
        all_embeddings = np.vstack(all_embeddings_list)
        all_images = np.vstack(all_images_list)

        distance_matrix = pairwise_distances(all_embeddings, metric="euclidean", n_jobs=None,force_all_finite=False)
    ...
    def generate_mining(self, embedding_model, n_classes, n_samples, margin=0.5, negative_selection_mode='semihard',
                        s='train'):
        while True:
