...
import pickle
from embedding_net.utils_emb import load_encodings
from embedding_net.backbones import get_backbone
from embedding_net.pretrain_backbone_softmax import pretrain_backbone_softmax
from embedding_net import losses_and_accuracies as lac
...
class EmbeddingNet:
    ...
    def predict_encoding(self,image):
        if type(image) is str:
            img = cv2.imread(image)
        else:
            img = image
