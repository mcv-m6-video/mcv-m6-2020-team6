...
class BBox:
    ...
    def contains_point(self, point: (float, float)) -> bool:
        return (
            self.top_left[0] <= point[0] <= self.get_bottom_right()[0]
            and self.top_left[1] <= point[1] <= self.get_bottom_right()[1]
        )
    ...
    def union(self, other: "BBox") -> "BBox":
        rec = BBox()
        rec.top_left = (
            min(self.top_left[0], other.top_left[0]),
            min(self.top_left[1], other.top_left[1]),
        )
        bottom_right = (
    ...
    def modify_bbox(self, noise):
        x = random.uniform(
            self.top_left[0] - self.top_left[1] * noise,
            self.top_left[0] + self.top_left[0] * noise,
        )
        y = random.uniform(
            self.top_left[1] - self.top_left[1] * noise,
            self.top_left[1] + self.top_left[1] * noise,
        )
        self.top_left = [x, y]
        self.width = random.uniform(
            self.width - self.width * noise, self.width + self.width * noise
        )
        self.height = random.uniform(
            self.height - self.height * noise, self.height + self.height * noise
        )
