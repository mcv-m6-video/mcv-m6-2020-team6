...
from metrics.evaluation_funcs import iou_overtime, iou_frame
...
def iou_over_time_RT_yolo(gt_dir, detections, begin, end):
    ...
    for i in range(num_framesyolo):
        ...
        for bbox_noisy in detections_bboxes.bboxes:
            ...
            plt.text(
                bbox_noisy.top_left[0],
                bbox_noisy.top_left[1],
                s="car",
                color="white",
...
def visualize_detections(
    detections_video, gt_video, begin, end, save=True, title="detections_"
):
    ...
    def animate(i):
        ...
        for bbox_noisy in detections_bboxes.bboxes:
            cv2.rectangle(
                img,
                (int(bbox_noisy.top_left[0]), int(bbox_noisy.top_left[1])),
                (
                    int(bbox_noisy.top_left[0] + bbox_noisy.width),
                    int(bbox_noisy.top_left[1] + bbox_noisy.height),
                ),
                (0, 0, 255),
                7,
            )
        ...
        for bbox_gt in gt_bboxes.bboxes:
            cv2.rectangle(
                img,
                (int(bbox_gt.top_left[0]), int(bbox_gt.top_left[1])),
                (
                    int(bbox_gt.top_left[0] + bbox_gt.width),
                    int(bbox_gt.top_left[1] + bbox_gt.height),
                ),
                (0, 255, 0),
                3,
            )
...
def visualize_detections_with_iou(
    detections_video, gt_video, begin, end, save=True, title="detections_"
):
...
def visualize_vtracking(
    tracking_video, gt, begin, end, path, save=True, title="tracking_"
):
    ...
    image = cv2.imread(path + "image" + str(begin).zfill(5) + ".jpg")
    ...
    def animate(i):
        ...
        images = path + "image" + str(begin + i).zfill(5) + ".jpg"
        img = cv2.imread(images)
        gt_bboxes = gt.get_frame_by_id(begin + i)
        data = tracking_video.get_frame_by_id(begin + i)
        for bbox in data.bboxes:
            color = label_color(bbox.det_id)
            caption_id = "ID: {}".format(bbox.det_id)
            cv2.rectangle(
                img,
                (int(bbox.top_left[0]), int(bbox.top_left[1])),
                (
                    int(bbox.top_left[0] + bbox.width),
                    int(bbox.top_left[1] + bbox.height),
                ),
                (0, 255, 0),
                3,
            )
            cv2.putText(
                img,
                caption_id,
                (int(bbox.top_left[0]), int(bbox.top_left[1] - 10)),
...
def visualize_tracking(
    ...
    image = cv2.imread(path + "frame" + str(begin) + ".jpg")
    ...
    def animate(i):
        ...
        images = path + "frame" + str(begin + i) + ".jpg"
        img = cv2.imread(images)
        gt_bboxes = gt.get_frame_by_id(begin + i)
        ...
        for bbox in data:
            ...
            cv2.rectangle(
                ...
                (int(bbox[1] + bbox[3]), int(bbox[2] + bbox[4])),
                color,
                7,
            )
            cv2.putText(
...
def visualize_tracking_kal(
    tracking_video, begin, end, path, save=True, title="tracking_"
):
    ...
    def animate(i):
        ...
        for bbox in data:
            ...
            if np.abs(bbox[7]) + np.abs(bbox[8]) > 0.5:
                cv2.arrowedLine(
                    img,
                    (int(bbox[5]), int(bbox[6])),
                    (
                        int(bbox[5]) + (int(bbox[7] * length)),
                        int(bbox[6]) + (int(bbox[8]) * length),
                    ),
                    color,
                    5,
                )
